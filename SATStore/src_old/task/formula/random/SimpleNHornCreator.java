...
public class SimpleNHornCreator extends FormulaCreator implements SATSolvable{
	...
	public SimpleNHornCreator(int numVars, double clauseVarRatio, int clauseSize) {
		super(numVars);
	...
	private void populateSAT(double clauseVarRatio, Conjunctions threeSat, ISolver satSolve) {
		...
		for(int i = 0; i < numVars*clauseVarRatio; i++) {
			...
			for(int j = 0; j < clauseSize; j++) {
				...
				if(j != 0 || rand.nextInt(2) == 1) {
					clauseForSolve[j] *= -1;
					l = l.negate();
				}

				thing[i][j] = clauseForSolve[j];
				clause.add(l);
			}
			threeSat.add(clause);
			try {
