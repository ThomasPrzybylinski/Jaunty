...
FileDecodable {
	...
	public AllFilledSquaresCNF(int size) {
		this.size = size;
	}
	...
	public String toString() {
		return "AllFilledSquaresCNF="+size;
	}
	...
	public CNF generateCNF(VariableContext context) {
		...
		for(int x = 0; x < size; x++) {
			for(int y = 0; y < size; y++) {
				...
				//These aren't necessary, but help make the formula more prime
//				addClauses(ret,x,y,curVar);
...
//		System.out.println(ret.getDeepSize());
	...
	private void addClauses(CNF ret, int x, int y, int curVar) {
		for(int x2 = 0; x2 < size; x2++) {
			for(int y2 = 0; y2 < size; y2++) {
				int otherVar = getVar(x2,y2);
				if(x2 == x && y2 == y) continue;
				if(otherVar < curVar) continue;

				int xDist = Math.abs(x-x2);
				int yDist = Math.abs(y-y2);

				int dirToX = x2 == x ? 1 : xDist/(x2-x);
				int dirToY = y2 == y ? 1 : yDist/(y2-y);

//				//We must ensure squareness
				if(xDist < yDist) {
					int vec = yDist*dirToX;
					boolean valid1 = validPt(x+vec,y);
					boolean valid2 = validPt(x-dirToX,y);

					if(valid1 && valid2) {
						ret.fastLocalSortAddClause(-curVar,-otherVar,getVar(x+vec,y),getVar(x-dirToX,y));						
					} else if(valid1) {
						ret.fastLocalSortAddClause(-curVar,-otherVar,getVar(x+vec,y));
					} else if(valid2) {
						ret.fastLocalSortAddClause(-curVar,-otherVar,getVar(x-dirToX,y));
					}



				} else if(yDist < xDist) {
					int vec = xDist*dirToY;
					boolean valid1 = validPt(x,y+vec);
					boolean valid2 = validPt(x,y-dirToY);
					if(valid1 && valid2) {
						ret.fastLocalSortAddClause(-curVar,-otherVar,getVar(x,y+vec),getVar(x,y-dirToY));						
					}
					else if(valid1) {
						ret.fastLocalSortAddClause(-curVar,-otherVar,getVar(x,y+vec));
					} else if(valid2) {
						ret.fastLocalSortAddClause(-curVar,-otherVar,getVar(x,y-dirToY));
					}


				}

				//ensure path between vars
				for(int xpath = 1; xpath <= xDist; xpath++) {
					ret.fastLocalSortAddClause(-curVar,-otherVar, getVar(x+xpath*dirToX,y));
					if(yDist != 0) {
						ret.fastLocalSortAddClause(-curVar,-otherVar, getVar(x2-xpath*dirToX,y2));
					}
				}

				for(int ypath = 1; ypath <= yDist; ypath++) {
					ret.fastLocalSortAddClause(-curVar,-otherVar, getVar(x,y+ypath*dirToY));
					if(xDist != 0) {
						ret.fastLocalSortAddClause(-curVar,-otherVar, getVar(x2,y2-ypath*dirToY));
					}
				}
	...
	private void ensureEquadistantCorners(CNF ret, int x, int y) {
		...
		if(validPt(x+1,y) && validPt(x,y+1)) {
			...
			for(int dist = 1; dist+y < size; dist++) {
				...
				if(validPt(x-1,botLeftY)) {
					map.put(getVar(x-1,botLeftY),0);
				}
				if(validPt(x,botLeftY+1)) {
					map.put(getVar(x,botLeftY+1),0);
				}
				...
				for(int k = 0; k < validCorners1.size(); k++) {
					...
					for(int i = 0; i < validCorners2.size(); i++) {
						...
						if(Arrays.equals(corner1,corner2)) {
							...
							if(validPt(x-corner1[0],y)) {
								map.put(getVar(x-corner1[0],y),0);
							}
							if(validPt(x,y-corner1[1])) {
								map.put(getVar(x,y-corner1[1]),0);
							}
							...
							if(validPt(x2-corner2[0],y2)) {
								map.put(getVar(x2-corner2[0],y2),0);
							}
							if(validPt(x2,y2-corner2[1])) {
								map.put(getVar(x2,y2-corner2[1]),0);
							}
	...
	private void onlyValidStructure(CNF ret, int x, int y) {
		if(onEdge(x)) {
			...
			//so if mid is surrounded, fill in mid
			ret.fastAddClause(-top,-left,mid);
			ret.fastAddClause(-top,mid,-right);
			ret.fastAddClause(-top,mid,-bot);
			ret.fastAddClause(-left,mid,-right);
			ret.fastAddClause(-left,mid,-bot);
