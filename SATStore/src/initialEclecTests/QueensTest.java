...
//		int[][] lens = PrototypesUtil.doAgreementSym(models,dnfForSym.getContext()); //12 of them
...
//		}
//		
//		for(int k = 0; k < globMods.size(); k++) {
//			int index1 = globMods.get(k);
//			if(!globMods.contains(k)) continue;
...
//		}
	...
	//	GraphizUtil.saveGraphAsPic(other,"QueenAgreementSymmetryGraph");
		...
		create.decodeAndSavePicture(origModels.get(0),"queensAgreement"+num+".png");
		num++;
		for(int k = 1; k < lens[0].length; k++) {
			if(lens[0][k] > 1) {
				...
				for(int[] perm : perms) {
					int[] otherModel = PermutationUtil.permuteClause(origModels.get(k),perm);
					for(int[] mod : models) {
						if(Arrays.equals(mod,otherModel)) {
							System.out.println(Arrays.toString(perm));
							System.out.println(create.decode(otherModel));
						}
					}
					
				}
				
				create.decodeAndSavePicture(origModels.get(k),"queensAgreement"+num+".png");
				num++;
			}
		}
