...
public class MaxIndRandCNFSyntaxTest {
...
//			CNFCreator creator = new IdentityCNFCreator("testcnf\\uf50-218\\uf50-0"+inst+".cnf");
			...
			boolean finishedOnce = true;
			double maxRand = Double.MIN_VALUE;
			for(int k = 0; k < radii.length; k++) {
				if(!finishedOnce) break;
				finishedOnce = false;

//				for(int j = 0; j < numRandClustTests; j++) {
//					ArrayList<int[]> randClust = new ArrayList<int[]>(curModels.size());
//					Collections.shuffle(allModels);
//
//					randClust.add(allModels.get(0));
//					for(int i = 1; i < allModels.size() && randClust.size() < curModels.size(); i++) {
//						int[] possible = allModels.get(i);
//						boolean add = true;
//						for(int[] comp : randClust) {
//							if(dist.lte(comp,possible,radii[k])) {
//								add = false;
//								break;
//							}
//						}
//
//						if(add) {
//							randClust.add(possible);
//						}
//					}
//					if(randClust.size() < curModels.size()) {
//						continue;
//					}
//					finishedOnce = true;
//					maxRand = Math.max(getMeanDist(randClust), maxRand);
//				}
				ArrayList<int[]> maxInd = getIndSet(allModels,k, curModels.size());
			}
			...
			for(int j = 0; j < numRandClustTests; j++) {
				...
				randRatio[0] += superRandDist/maxRand;
				curRatioSum[0] += curDist/maxRand;
				allModelsSize[0] += allModels.size();
				SymSizeRatio[0] += curModels.size();
				clustSizeRatio[0] += superRand.size();
				totalLTMaxModel[0]++;
			}

			System.out.println(inst);
			//			for(int k = 0; k < radii.length; k++) {
			System.out.print(radii[0]+"\t");
			System.out.print((curRatioSum[0]/(double)totalLTMaxModel[0])+"\t");
			//				System.out.print((numCurHasBetter[k])+"\t");//(double)(inst*numRandClustTests))+"\t");
			//				System.out.print((adjRatio[k]/(double)totalLTMaxModel[k])+"\t");
			System.out.print((randRatio[0]/(double)totalLTMaxModel[0])+"\t");
			//				System.out.print((randAdjRatio[k]/(double)totalLTMaxModel[k])+"\t");
			System.out.print(SymSizeRatio[0]/(double)totalLTMaxModel[0]+"\t");
			//				System.out.print(clustSizeRatio[k]/(double)totalLTMaxModel[k]+"\t");
	...
	private static ArrayList<int[]> getIndSet(ArrayList<int[]> allModels, int k, int maxSize) {
