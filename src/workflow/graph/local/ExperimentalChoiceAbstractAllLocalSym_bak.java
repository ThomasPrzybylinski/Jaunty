...
//of literals is not
public abstract class ExperimentalChoiceAbstractAllLocalSym_bak extends ReportableEdgeAddr {
...
//	private LiteralPermutation modID;
	...
	public ExperimentalChoiceAbstractAllLocalSym_bak(ChoiceGetter choice) {
		super();
		choices = choice;
	}

	
	public ExperimentalChoiceAbstractAllLocalSym_bak() {}


	public ExperimentalChoiceAbstractAllLocalSym_bak(boolean checkFirstInLocalOrbit,
	...
	public ExperimentalChoiceAbstractAllLocalSym_bak(boolean checkFirstInLocalOrbit, boolean checkLitGraph,
...
//			nextCanon = choices.getChoiceInterp(nextCanon);
			...
			for(int i : allInNext) {
				if(!choices.isChoice(i)) continue;
			...
			if((prevFilter.length > 0 && Math.abs(prevFilter[prevFilter.length-1]) > Math.abs(next)) 
				...
				sc = getShortcut(choices.getChoiceInterp(nextCanon),varID,modID);
...
//					SchreierVector vec = new SchreierVector(info.getLast().getSyms());
					...
					if(rep != next && choices.isChoice(rep)) {
...
//					}
				...
				if(smallerPerm != null) {
					int[] nextSmaller = smallerPerm.applySort(choices.getChoiceInterp(nextCanon));
//					LiteralPermutation compModPerm = assocModelPerm;
					...
					try{
						sc = getShortcut(choices.getChoiceInterp(nextSmaller),smallerPerm.inverse(),compModPerm );
...
//			pastInfo.lp.modelGroup = pastInfo.lp.modelGroup.reduce();
	...
	private Shortcut getShortcut(int[] nextCanon,
		...
		int prevIndex = -1;
		while(!Arrays.equals(lookingFor,choices.getChoiceInterp(cur.filter))) {
			int[] filter = choices.getChoiceInterp(cur.filter);
			debug.add(cur);
			...
			if(sc.litPerm.isId()) {
				...
				lookingFor = choices.getChoiceInterp(sc.litPerm.inverse().applySort(lookingFor));
				index = calibrateIndex(cur, lookingFor, index);
	...
	private int calibrateIndex(LatticePart cur, int[] lookingFor, int index) {
		int k = index;
		int[] filter = choices.getChoiceInterp(cur.filter);
		for(; k < filter.length; k++) {
			if(lookingFor[k] !=filter[k]) {
				return k;
			}
		}
		return k;
