...
public abstract class AbstractExperimentalAllLocalSym extends ReportableEdgeAddr {
...
//	private LiteralPermutation modID;
	...
	public AbstractExperimentalAllLocalSym() {}


	public AbstractExperimentalAllLocalSym(boolean checkFirstInLocalOrbit,
	...
	public void addEdges(PossiblyDenseGraph<int[]> g, ClauseList orig) {
		...
		agree.addEdges(g,orig);
		
		this.iters += agree.iters;
...
//		boolean usefulToContinue = getUsefulToContinue(g, clauses);
//		if(!usefulToContinue) return;
		...
		for(int next : validLits) {
			...
			//If we found everything that's going to be found, just continue!
			boolean usefulToContinue = getUsefulToContinue(g, clauses);
			if(!usefulToContinue) break;
...
//					LiteralPermutation compModPerm = assocModelPerm;
			...
			if(sc != null) {
				...
				for(IntPair p : sc.part.getPairs()) {
					...
					g.setEdgeWeight(newPair.getI1()-1,newPair.getI2()-1,0);
				}
			}

			if(sc == null) {
				findSyms(g,nextFilter, nextCanon, clauses,litGraph,info, next);
			}
			clauses.pop();
		}
	...
	private boolean getUsefulToContinue(PossiblyDenseGraph<int[]> g,
			LocalSymClauses clauses) {

		for(int k = 0; k < clauses.numTotalModels(); k++) {
			if(!clauses.isClauseValid(k)) continue;
			
			for(int i = k+1; i < clauses.numTotalModels(); i++) {
				if(!clauses.isClauseValid((i))) continue;
					
				if(g.getEdgeWeight(k,i) != 0) {
					return true;
				}
			}
		}
	...
	private void findSyms(PossiblyDenseGraph<int[]> g,int[] filter, int[] canonFilter, LocalSymClauses clauses,
			DirectedLitGraph litGraph, LinkedList<LocalInfo> info, int lastChoice) {
		iters++;
...
//				System.out.println(Arrays.toString(canonFilter));
		...
		if(numModels > 1) {
			...
			finder.addKnownSubgroup(parentInfo.getSyms().getStabSubGroup(lastChoice).reduce());
...
//						System.out.println(modelGroup.reduce());
			...
			computeCurModels(info.getLast().lp,g);
...
//	}
	...
	protected abstract void computeCurModels(LatticePart lp, PossiblyDenseGraph<int[]> g);
