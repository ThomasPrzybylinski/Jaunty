...
public abstract class AbstractLimitedLocalSym extends ReportableEdgeAddr {
...
//	private LiteralPermutation modID;
	...
	public AbstractLimitedLocalSym() {}


	public AbstractLimitedLocalSym(boolean checkFirstInLocalOrbit,
...
//				System.out.println(Arrays.toString(canonFilter));
		...
		if(numModels > 1) {
			...
			finder.addKnownSubgroup(parentInfo.getSyms().getStabSubGroup(next).reduce());
...
//			addInCorrectPlace(latP,canonFilter);
...
//			debug.add(cur);
	...
	private void addInCorrectPlace(LatticePart latP, int[] filter) {
		LatticePart cur = root;
		LatticePart prev = root;
		int[] lookingFor = filter;
		int index = 0;
		index = calibrateIndex(cur, lookingFor, index);
		LiteralPermutation literalPermutation = varID;
		CompressedModelPermutation modelPermutation = modID;
		...
		while(!Arrays.equals(lookingFor,cur.filter)) {
			debug.add(cur);
			if(index <= -1)  {
				prev.addChild(filter[-index], new Shortcut(varID,modID,latP));
				return;
			}
			int nextInt = lookingFor[index];
			Shortcut sc = cur.children.get(nextInt);
			...
			if(sc == null)
			{
				sc = new Shortcut(varID,modID,latP);
				cur.addChild(nextInt,sc);
				return;
				
			} else if(sc.litPerm.isId()) {
				prev = cur;
				cur = sc.part;
	...
	private int calibrateIndex(LatticePart cur, int[] lookingFor, int index) {
		return calibrateIndex(cur,lookingFor,index,false);
	}


	private int calibrateIndex(LatticePart cur, int[] lookingFor, int index, boolean canAdd) {
		int k = index;
		for(; k < cur.filter.length && k < lookingFor.length; k++) {
			if(lookingFor[k] != cur.filter[k]) {
				if(!canAdd && Math.abs(cur.filter[k]) > Math.abs(lookingFor[k])) return -k;
				return k;
			}
		}
		
		if(lookingFor.length < cur.filter.length && k == lookingFor.length) {
