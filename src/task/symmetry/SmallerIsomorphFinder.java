...
public class SmallerIsomorphFinder {
	...
	private LiteralPermutation getSmallerSubsetIfPossible(int[] curSet, int curIndex,
		...
		while(comp.compare(curMapping, curSet[curIndex]) < 0) {
			...
			Set<Integer> mappings = getPossibleMappingsTo(vec,stabGroupMapping);
	...
	public static Set<Integer> getPossibleMappingsTo(SchreierVector vec, int num) {
