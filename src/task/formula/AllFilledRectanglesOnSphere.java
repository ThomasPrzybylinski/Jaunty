...
public class AllFilledRectanglesOnSphere implements ModelGiver, ConsoleDecodeable, FileDecodable {
	private int size;

	public AllFilledRectanglesOnSphere(int size) {
		this.size = size;
	}
	...
	public DNF generateDNF(VariableContext context) {
					...
					for(int y = 0; y < size; y++) {
						...
						for(int x2 = 0; x2 < size; x2++) {
							for(int y2 = 0; y2 < size; y2++) {
								...
								if(isOnRectangle(x,y,sx,sy,x2,y2)) {
		...
		return new DNF(dis).reduce();
	}

	private boolean isOnRectangle(int x, int y, int sizex, int sizey, int x2, int y2) {
		 return ((x2 >= x && x2 <= x+sizex) || (x2 < x && (x2+size) <= x+sizex))
				 && ((y2 >= y && y2 <= y+sizey) || (y2 < y && (y2+size) <= y+sizey))
				//&& (x2 == x || x2 == (x+sizex)%size || y2 ==y || y2 == (y+sizey)%size)
				;
	}
	...
	public String toString() {
		return "AllFilledRectangesOnSphere="+size;
	}
