...
public class ComparisonSyntaxTest {
	...
	public static void main(String[] args) throws Exception {
		final int maxDiv = 100;//Integer.MAX_VALUE;//;//20;
...
//						CNFCreator creator = new IdentityCNFCreator("testcnf\\uf200-860\\uf200-0"+13+".cnf");
		...
		doOnlineSym(maxDiv, maxTime, creator, f, function, origVars,true);
	}

	private static void doOnlineSym(final int maxDiv, final long maxTime,
			CNFCreator creator, File f, CNF function, int origVars, boolean doSymBreak)
...
//						finder.addKnownSubgroup(new NaiveLiteralGroup(rejPerm));
				...
				if(!add && !doSymBreak) {// && solver.nConstraints() <= 10*origCNF.size()) {
					break;
				}
			}
			if(add) {
